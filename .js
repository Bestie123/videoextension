...
document.getElementsByClassName('video_items_list ').forEach((videolist) => {
	                //пArray.prototype.difference = function symmetricDierence(a1) { // проверяет есть ли в первом массиве совпадения со вторым массивом, удаляет из первого совпавшие элементы и возвращает первый массив
    return this.filter(x => !a1.includes(x));
}
...
роверять стоят ли обработчики на данном списке, чтоб не вешать их повторно	
	if (visitsCountMap.has(videolist) == false) {
    listItem2 = videolist.children;
    listArray2 = [...listItem2];
    listArray2.forEach((item) => {
