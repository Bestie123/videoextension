            ...
            document.getElementsByClassName('video_items_list ').forEach((videolist) => {
                ...
                listArray23.forEach((item1111) => {
                    ...
                    if (ggggg == "null" || ggggg == null || ggggg == 0 || ggggg == "0" || ggggg == '0') {} else {
						...
						//item1111.getAttribute("data-hash");
...
addremovevideo = function(param) {
					...
					var a4=llll4.toString();
					 param.forEach((item) => {
					//	 a1 = a1 + videolist.album_id+'|'; //должно быть фиксированное постоянное для всех видео
                    ...
                    Bestie123webmodule.api('execute.addvideos', {album_ids:a1, owner_id:a2, video_id:a3}).then(function(values) {// альбомы для добавления
					console.log(values);
                        ...
                        Bestie123webmodule.api('execute.removevideos',  {album_ids:a4, owner_id:a2, video_id:a3}, function(r) {}).then(function(values) {//альбомы для удаления
                            console.log(values);
                ...
                var loc333 = function(param) {
					...
					var a5=[];
					 param.forEach((videolist) => {
						 console.log(videolist.album_id);
                    ...
                    Bestie123webmodule.api('execute.sortVideos', {album_id:a1, owner_id:a2, video_id:a3, after_owner_id:a4, after_video_id:a5}).then(function(values) {
                        console.log(values);
                                        ...
                                        nestedparentvideo = function() {
                                            if (parentvideoindxcount > 0) {
                                                ...
                                                if (item2 in parentvideosarray[parentvideoindxcount].newaddedalbums) {
													...
													//	hol4.push(a4);
													//	hol5.push(a5);
                                                    parametrspostarray2.push({album_id:a1, owner_id:a2, video_id:a3, after_owner_id:a4, after_video_id:a5});
                                                } else {
                                                    nestedparentvideo()
                                                }
                                            }
				...
				//	]
                    ...
                    if (parametrspostarray2.length == 0) {
                     ...
                     //   parametrspostarray2 = parametrspostarray2.reverse()
												// hol1 = spl(hol1,25); //максиммальное количество видео которое может вернуть запрос
						// hol2 = spl(hol2,25); //максиммальное количество видео которое может вернуть запрос
					//	 hol3 = spl(hol3,25); //максиммальное количество видео которое может вернуть запрос
					//	 hol4 = spl(hol4,25); //максиммальное количество видео которое может вернуть запрос
					//	 hol5 = spl(hol5,25); //максиммальное количество видео которое может вернуть запрос
...
//	zk2=Bestie123webmodule;
//	console.log(typeof(Bestie123webmodule.setApiConfig))
//setTimeout(function(){
	//Bestie123webmodule.removeLoginCache()
//})
	 Bestie123webmodule.setApiConfig({
        appId: 8182193,
        version: '5.81',
